...
namespace :db do
  ...
  task :populate => :environment do
    ...
    lacey_note_2 = FactoryBot.create(:investigation_note, investigation: lacey, officer: jblake, content: "I found the weapon in a dumpster at the waterfront.")
    ...
    puts "Creating 900 past crimes ... takes some time"
    300.times do
      days_back = (40..400).to_a.sample
      days_to_solve = (3..30).to_a.sample
      days_closed_on = days_back - days_to_solve
      street_address = Faker::Address.street_name
      ...
      this_ci = FactoryBot.create(:crime_investigation, crime: this_crime, investigation: this_investigation)
      officer_assignment = FactoryBot.create(:assignment, investigation: this_investigation, officer: investigator)
      officer_assignment.update_attribute(:start_date, days_back.days.ago.to_date)
      this_investigation.update_attribute(:date_closed, days_closed_on.days.ago.to_date)
      officer_assignment.update_attribute(:end_date, days_closed_on.days.ago.to_date)
    end

    600.times do
      days_back = (5..500).to_a.sample
      days_to_solve = (1..3).to_a.sample
      days_closed_on = days_back - days_to_solve
      street_address = Faker::Address.street_name
      investigator = patrol_officers.sample
      this_crime = misdemeanors.sample
      this_investigation = FactoryBot.create(:investigation, title: "Crime at #{street_address}", description: "A #{this_crime.name.downcase} committed at #{street_address}.", crime_location: street_address, date_opened: days_back.days.ago.to_date, date_closed: nil, solved: false, batman_involved: true)
      this_ci = FactoryBot.create(:crime_investigation, crime: this_crime, investigation: this_investigation)
      officer_assignment = FactoryBot.create(:assignment, investigation: this_investigation, officer: investigator)
      officer_assignment.update_attribute(:start_date, days_back.days.ago.to_date)
      this_investigation.update_attribute(:date_closed, days_closed_on.days.ago.to_date)
    ...
    # And a few current misdemeanors for patrol
    40.times do
      days_back = (1..5).to_a.sample
      street_address = Faker::Address.street_name
      investigator = patrol_officers.sample
      this_crime = misdemeanors.sample
      this_investigation = FactoryBot.create(:investigation, title: "Crime at #{street_address}", description: "A #{this_crime.name.downcase} committed at #{street_address}.", crime_location: street_address, date_opened: days_back.days.ago.to_date, date_closed: nil, solved: false, batman_involved: true)
    ...
    ### FINAL STEP: Adding some phase 5 testing cases
    # MCU officer with no cases assigned
    akasinsky_user = FactoryBot.create(:user, username: "akasinsky", role: "officer")    
    akasinsky = FactoryBot.create(:officer, first_name: "Andi", last_name: "Kasinsky", rank: "Detective", user: akasinsky_user, unit: major_crimes)

    # Some unassigned cases
    ...
    murder1a_ci = FactoryBot.create(:crime_investigation, crime: murder1, investigation: murder1a_investigation)

    # Increase in drug trafficking over last 30 days
    mcu_narc_combo = narcotics_officers + mc_officers
    25.times do
      days_back = (1..29).to_a.sample
      street_address = Faker::Address.street_name
      investigator = mcu_narc_combo.sample
      narc_investigation = FactoryBot.create(:investigation, title: "Narcotics trafficking at #{street_address}", description: "Drug trafficking at #{street_address}.", crime_location: street_address, date_opened: days_back.days.ago.to_date, date_closed: nil, solved: false, batman_involved: false)
      narc_ci = FactoryBot.create(:crime_investigation, crime: traf, investigation: narc_investigation)
      officer_assignment = FactoryBot.create(:assignment, investigation: narc_investigation, officer: investigator)
    end  

    # Increase in drug possession over last 80 days
    140.times do
      days_back = (1..80).to_a.sample
      street_address = Faker::Address.street_name
      investigator = narcotics_officers.sample
      narc_investigation = FactoryBot.create(:investigation, title: "Drug bust at #{street_address}", description: "Drug possession found at #{street_address}.", crime_location: street_address, date_opened: days_back.days.ago.to_date, date_closed: nil, solved: false, batman_involved: false)
      narc_ci = FactoryBot.create(:crime_investigation, crime: possess, investigation: narc_investigation)
      officer_assignment = FactoryBot.create(:assignment, investigation: narc_investigation, officer: investigator)
    end
    puts "Added phase 5 test cases"
  end
end
